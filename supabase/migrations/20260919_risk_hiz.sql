-- ogrenci_risk hızlandırma (19 Eylül 2026). Öğrenciler listesi 25 öğrenciyle
-- 5 saniyede açılıyordu: görünüm security_invoker olduğu için içindeki her
-- tabloda (gorevler, oturumlar, denemeler...) RLS her satırda iki fonksiyon
-- çağırıyordu; 26 öğrencide sorgu tek başına ~1,1 sn, öğrenci sayısıyla
-- doğrusal büyüyordu. Artık erişim kontrolü öğrenci başına bir kez,
-- görünümün başında yapılıyor (aynı kural: private.erisebilir_mi — kendisi,
-- koçu, velisi, yönetici); içerideki tablolar bu öğrencilerle süzülüyor.
create or replace view public.ogrenci_risk with (security_invoker = false) as
 WITH aktiflik AS MATERIALIZED (
         SELECT o.id AS ogrenci_id, o.koc_id, o.kayit_tarihi,
           ...
           FROM ogrenciler o
          WHERE o.aktif AND private.erisebilir_mi(o.id)
        ), hafta AS (
         SELECT gorevler.ogrenci_id,
           ...
           FROM gorevler
          WHERE gorevler.ogrenci_id IN (SELECT ogrenci_id FROM aktiflik)
            AND gorevler.tarih >= (private.yerel_gun(now()) - 7) AND gorevler.tarih <= private.yerel_gun(now())
         ...
         SELECT d.ogrenci_id, d.tur, d.id AS deneme_id, d.tarih, sum(s.net) AS toplam_net,
          ...
          WHERE d.ogrenci_id IN (SELECT ogrenci_id FROM aktiflik)
...
-- Görünüm artık sahibinin haklarıyla çalışıyor; oturumsuz (anon) erişime gerek yok.
revoke select on public.ogrenci_risk from anon;
